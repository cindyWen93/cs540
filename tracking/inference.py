...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while i < len(self.legalPositions):
            if(j == self.numGhosts):
                ...
                newList = []
                j = 0
            newList.append(self.legalPositions[i])
            i += 1
            j += 1
        '''
        i = 0
        ...
        while i < self.numGhosts:
			myList.append(self.legalPositions)
			i += 1
        self.particles = [a for a in itertools.product(*myList)]
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if (noisyDistances[i] != None):
                    myNienie *= emissionModels[i][util.manhattanDistance(particle[i], pacmanPosition)]
                else:
                    particle = self.getParticleWithGhostInJail(particle , i)
            allPossible[particle] += myNienie
        ...
        if allPossible.totalCount() == 0:
            ...
            i = 0
            while i < self.numParticles: 
                nienie.append(util.sample(allPossible))
                i += 1
           
            self.particles = nienie
    ...
    def elapseTime(self, gameState):
        ...
        #
            newParticle = [util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), i, self.ghostAgents[i])) for i in range(self.numGhosts)]
            "*** END YOUR CODE HERE ***"
